...
def main():
    ...
    slb_server_partition = client.slb.system.partition.active(partition)
    ...
    slb_result = client.slb.service_group.get(slb_service_group)
    ...
    if state == 'present':
        ...
        for server in slb_servers:
            result = client.slb.server.get(server['server'])
